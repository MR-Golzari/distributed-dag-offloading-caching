...
def read_value_txt_files(top_level_folder,search):
    ...
    #betalist= ['0','10','20','30','40','70','100','150','200']
...
def plot_data(data_dict, max_indices, window_size,tag):
    ...
    plt.tight_layout()
    plt.savefig('with_'+tag+'.pdf',dpi=300)
    plt.savefig('with_'+tag+'.png')
    plt.show()
    tele = Telegram()
    tele.send_photo('with_'+tag+'.png',tag)
def main(directory, max_indices, window_size):
    """Main function to read files  froma directory and plot the moving average of the data."""
    data_list = []
    labels = []
    ...
    #tag = "velocity_prev_servers_plus_service_per_serverDQN_v_"
    tag = "withoutfederated_prev_servers_plus_service_per_serverDQN_v_"
...
if __name__ == "__main__":
    ...
    window_size = 5000  # Set the window size for the moving average
